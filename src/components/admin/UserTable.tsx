      ...
      <div className="bg-white border border-gray-200 rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="px-6 py-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input
              ...
              className="bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          ...
          {isSuperAdmin && (
            <div className="flex flex-col gap-1.5">
              ...
              <select
                ...
                className="bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100">
          <button
            ...
            onClick={onClose}
            disabled={isPending}
            className="px-4 py-2 text-sm text-gray-500 hover:text-gray-900 bg-white border border-gray-300 rounded-lg transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <input
          ...
          className="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <select
          value={filterRole}
          ...
          className="bg-white border border-gray-300 rounded-lg px-4 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        >
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 text-gray-400 bg-white border border-gray-200 rounded-xl shadow-sm">
              ...
              {filtered.map((user) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Rol */}
                    <td className="px-5 py-4">
                      <span className={`text-xs px-2.5 py-0.5 rounded-full ${ROLE_COLORS[user.role] ?? "bg-gray-100 text-gray-600"}`}>
                        {ROLE_LABELS[user.role] ?? user.role}
                      </span>
                    </td>

                    {/* Instrumento */}
                    <td className="px-5 py-4 hidden md:table-cell text-gray-700 text-sm">
                      {user.instrument ? (
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {canEdit && !isSelf && (
                          <button
                            ...
                            disabled={isPending}
                            className="text-xs px-3 py-1.5 bg-white hover:bg-indigo-50 border border-gray-300 hover:border-indigo-400 text-gray-600 hover:text-indigo-600 rounded-lg transition-colors disabled:opacity-50"
                          >
                            Editar
                          </button>
                        )}
                        {isSuperAdmin && !isSelf && (
                          <button
                            ...
                            disabled={isPending}
                            className="text-xs px-3 py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 rounded-lg transition-colors disabled:opacity-50"
                          >
                            Eliminar
                          </button>
                        )}
